...
const About = () => (
  <section id="about" className="bg-bg" style={{ padding:"7rem 5%", position:"relative" }}>
    <style>{`
      ...
      .skill-chip:hover { border-color:rgba(0,245,160,.3); color:#f0f0f8; }
    `}</style>
    ...
    <div className="about-grid">
      ...
      <div className="reveal" style={{ position:"relative", height:460 }}>
        <div style={{ position:"absolute", inset:0, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:24, overflow:"hidden", display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse 80% 50% at 50% 20%, rgba(0,245,160,.07), transparent), radial-gradient(ellipse 60% 60% at 80% 80%, rgba(0,200,255,.05), transparent)", pointerEvents:"none" }} />
          <div style={{ position:"relative", zIndex:2, fontFamily:"'Courier New', monospace", fontSize:"0.78rem", lineHeight:1.8, color:"rgba(240,240,248,0.6)", padding:"2rem", width:"100%" }}>
            ...
            {'  '}mission:{' '}<span style={{ color:"#00f5a0" }}>"ship things that work"</span>,<br />
            {'  '}values:{' ['}
              <br />{'    '}<span style={{ color:"#00f5a0" }}>"clarity"</span>,
              <br />{'    '}<span style={{ color:"#00f5a0" }}>"speed"</span>,
              <br />{'    '}<span style={{ color:"#00f5a0" }}>"quality"</span>,
              <br />{'    '}<span style={{ color:"#00f5a0" }}>"honesty"</span>
            <br />{'  '}{'],'}<br />
            {'  '}stack:{' '}<span style={{ color:"#00f5a0" }}>"whatever the job needs"</span>,<br />
            {'  '}deliveryMode:{' '}<span style={{ color:"#00f5a0" }}>"relentless"</span>,<br />
            {'  '}bullshit:{' '}<span style={{ color:"#00f5a0" }}>false</span>,<br />
            {'};'}<br /><br />
        ...
        <div style={{ position:"absolute", top:32, left:-24, display:"flex", alignItems:"center", gap:10, background:"rgba(5,5,8,.95)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:14, padding:"0.7rem 1.25rem", backdropFilter:"blur(12px)", boxShadow:"0 8px 32px rgba(0,0,0,.5)", fontSize:"0.82rem", fontWeight:500, color:"#f0f0f8", whiteSpace:"nowrap" }}>
          <span className="anim-pulse" style={{ width:8, height:8, borderRadius:"50%", background:"#00f5a0", boxShadow:"0 0 8px #00f5a0", flexShrink:0 }} />
