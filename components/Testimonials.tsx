...
const Testimonials = () => (
  <section id="testimonials" className="bg-bg2" style={{ padding:"7rem 5%", position:"relative" }}>
    <style>{`
      ...
      .testi-card:hover { border-color:rgba(0,245,160,.2); }
    `}</style>
    ...
    <div className="testi-grid">
      {testimonials.map((t, i) => (
        <div key={t.name} className={`testi-card reveal reveal-delay-${i}`}>
          {/* Big quote */}
          <div style={{ position:"absolute", top:12, right:20, fontFamily:"Georgia, serif", fontSize:"3.5rem", lineHeight:1, color:"#00f5a0", opacity:0.18, pointerEvents:"none", userSelect:"none" }}>"</div>
          {/* Stars */}
          <div style={{ color:"#00f5a0", fontSize:"0.875rem", marginBottom:"1rem", letterSpacing:"0.05em" }}>★★★★★</div>
          <p style={{ color:"rgba(240,240,248,0.8)", fontSize:"0.9rem", lineHeight:1.75, marginBottom:"1.5rem" }}>{t.text}</p>
          <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
            <div style={{ width:40, height:40, borderRadius:"50%", background:"linear-gradient(135deg, #00f5a0, #00c8ff)", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:"0.85rem", color:"#020906", flexShrink:0 }}>{t.initials}</div>
            <div>
